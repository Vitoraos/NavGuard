...
async function checkInsideSafeAirspace(
  ...
  // BUG-02: comparing MSL altitude to MSL session limits
  const withinVertical = altitudeMSL_ft >= sessionLimits.floor && altitudeMSL_ft <= sessionLimits.ceiling;

  return {
    inside:      insideStored && !newTfrHit && withinVertical,
...
export async function checkDronePosition(
  ...
  const [safeAirspaceCheck, currentWeather, timeline] = await Promise.all([
    ...
    getWeatherAtPosition(current, altitudeMSL_ft, sessionLimits.ceiling),
    queryWeatherTimeline(current, destination, new Date(), remainingSeconds, sessionLimits.ceiling),
  ]);
