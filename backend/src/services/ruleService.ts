...
export async function queryAirspace(
  ...
  const sql = `
    ...
    tfr_zones AS (
      ...
      WHERE altitude_ceiling >= $3 AND altitude_floor <= $4 
  ...
  const params = [
    ...
    WEATHER_CELL_RADIUS_M, thresholds.max_wind_mph, thresholds.max_precip, thresholds.min_visibility,
    selectBand(ceil), origin.lon, origin.lat, destination.lon, destination.lat,
  ];
  const restrictionParams = [polylineGeoJSON, bufferMeters, floor, ceil, flightStart.toISOString(), flightEnd.toISOString()];
  ...
  if (!row) {
    return { safe_airspace: null, safe_fragments: null, corridor: { type: "LineString", coordinates: [[origin.lon, origin.lat], [destination.lon, destination.lat]] }, no_fly: { regulatory: null, weather: null }, restrictions: [], path_connected: false };
  }
  ...
  const restrictions = restrictionsResult.rows.map((r: any) => ({
    ...
    properties: { id: r.id, name: r.name, reason: r.reason, type: r.type, altitude_floor: r.altitude_floor, altitude_ceiling: r.altitude_ceiling, source: r.source, start_time: r.start_time, end_time: r.end_time },
  }));
  ...
  return {
    ...
    no_fly: { regulatory: row.tfr_no_fly ? JSON.parse(row.tfr_no_fly) : null, weather: row.weather_no_fly ? JSON.parse(row.weather_no_fly) : null },
    restrictions,
...
export async function queryWeatherTimeline(
  ...
  const sql = `
    WITH flight_line AS ( SELECT ST_SetSRID(ST_MakeLine(ST_MakePoint($1, $2), ST_MakePoint($3, $4)), 4326) AS geom ),
