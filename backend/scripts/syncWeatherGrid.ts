...
// FIX-09b: doubled grid density for sub-15km resolution
...
await pool.query(`INSERT INTO public.weather_grid (cell_id, geom, wind_mph, precip, visibility, fetched_at) SELECT cell_id, ST_SetSRID(ST_MakePoint(lon, lat), 4326), wind, precip, vis, fetched::timestamptz FROM UNNEST( $1::bigint[], $2::float[], $3::float[], $4::float[], $5::float[], $6::float[], $7::text[] ) AS t(cell_id, lat, lon, wind, precip, vis, fetched) ON CONFLICT (cell_id, fetched_at) DO UPDATE SET wind_mph = EXCLUDED.wind_mph, precip = EXCLUDED.precip, visibility = EXCLUDED.visibility, geom = EXCLUDED.geom`, [
...
const { rowCount } = await pool.query(`DELETE FROM public.weather_grid WHERE fetched_at < NOW() - INTERVAL '50 minutes'`);
return rowCount ?? 0;
}
// ─── Main ─────────────────────────────────────────────────────────────────────
async function main() {
