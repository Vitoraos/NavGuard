...
export default function CTA() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const res = await fetch('/api/captureLead', { method: 'POST', body: JSON.stringify(body), headers: {'Content-Type': 'application/json'}});
    if (res.ok) setStatus('Thank you! Check your email for docs.');
  ...
  return (
    <section id="cta" className="h-screen flex flex-col justify-center items-start px-12 space-y-6 bg-primary text-white">
      ...
      <form className="flex flex-col space-y-4 mt-4 w-full max-w-md" onSubmit={handleSubmit}>
        ...
        <button type="submit" className="bg-accent text-black font-bold py-3 rounded hover:opacity-90 transition">
          Access Docs
        </button>
      </form>
